...
export const Send: FC<{ identity: IdentityAlias }> = ({ identity }) => {
  ...
  const sendPayment = async () => {
    ...
    if (clusterURL != undefined && publicKey != undefined && sendAmount != '') {
      ...
      console.log(
        'unable to send, either the cluster URL, send amount, or public key are not set',
      )
    }
  }
        ...
        <label htmlFor="add-proof-modal" className="modal-box m-3">
          ...
          <div className="mt-8 stats shadow w-full text-center">
            <div className="stat">
              ...
              <div className="stat-desc">
                {identity.owner.slice(0, 6)}...
                {identity.owner.slice(identity.owner.length - 6, identity.owner.length)}
              </div>
            </div>
          </div>
          <div className="form-control w-full">
            <label className="label">
              ...
              <span
                className="label-text-alt badge badge-primary cursor-pointer"
                onClick={setSendAmountMax}
              >
            ...
            <input
              ...
              value={sendAmount}
              onChange={setSendAmountInput}
            />
            <label className="label">
              <span className="label-text-alt">
                the transaction will be confirmed before any funds are sent
              </span>
            </label>
          </div>
